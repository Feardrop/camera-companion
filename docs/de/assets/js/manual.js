...
const printedLabel = pdf => { const pr = pdf - OFFSET; return pr >= 1 ? t("manualPagePrefix") + " " + pr : t("manualFrontMatter", { n: pdf }); };
...
function doSearch(){
  ...
  if(!hits.length){ out.innerHTML = '<p class="mut">' + t("noResultsFor", { q: esc(q) }) + '</p>'; return; }
  out.innerHTML = '<p class="mut">' + t("manualHitsCount", { n: hits.length }) + '</p>' + hits.map(h=>{
